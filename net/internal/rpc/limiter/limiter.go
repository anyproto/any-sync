//go:generate mockgen -destination mock_limiter/mock_limiter.go github.com/anyproto/any-sync/net/internal/rpc/limiter RpcLimiter
package limiter
...
func (h *limiter) HandleRPC(stream drpc.Stream, rpc string) (err error) {
	peerId, err := peer2.CtxPeerId(stream.Context())
	if err != nil {
		return err
	}
