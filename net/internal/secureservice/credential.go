...
func newNoVerifyChecker(protoVersion uint32, compatibleProtoVersions []uint32, clientVersion string) handshake2.CredentialChecker {
...
func (n noVerifyChecker) CheckCredential(remotePeerId string, cred *handshakeproto.Credentials) (result handshake2.Result, err error) {
	if !slices.Contains(n.compatibleVersions, cred.Version) {
		err = handshake2.ErrIncompatibleVersion
		return
	}
...
func newPeerSignVerifier(protoVersion uint32, compatibleProtoVersions []uint32, clientVersion string, account *accountdata.AccountKeys) handshake2.CredentialChecker {
...
func (p *peerSignVerifier) CheckCredential(remotePeerId string, cred *handshakeproto.Credentials) (result handshake2.Result, err error) {
	if !slices.Contains(p.compatibleVersions, cred.Version) {
		err = handshake2.ErrIncompatibleVersion
		return
	}
	if cred.Type != handshakeproto.CredentialsType_SignedPeerIds {
		err = handshake2.ErrSkipVerifyNotAllowed
		return
	}
	...
	if err != nil {
		err = handshake2.ErrInvalidCredentials
		return
	}
