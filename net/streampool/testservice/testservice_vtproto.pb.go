...
func (m *StreamMessage) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *StreamMessage) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *StreamMessage) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.ReqData) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.ReqData)))
		i--
		dAtA[i] = 0xa
	}
...
func (m *StreamMessage) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *StreamMessage) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
