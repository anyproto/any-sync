...
func (m *PayloadSignedPeerIds) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *PayloadSignedPeerIds) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *PayloadSignedPeerIds) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.Sign) > 0 {
		i -= len(m.Sign)
		copy(dAtA[i:], m.Sign)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Sign)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.Identity) > 0 {
...
func (m *Proto) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *Proto) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *Proto) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.Encodings) > 0 {
		var pksize2 int
		for _, num := range m.Encodings {
			pksize2 += protohelpers.SizeOfVarint(uint64(num))
		}
		i -= pksize2
		j1 := i
		for _, num1 := range m.Encodings {
			num := uint64(num1)
			for num >= 1<<7 {
...
func (m *PayloadSignedPeerIds) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *PayloadSignedPeerIds) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
...
func (m *Proto) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		case 2:
			if wireType == 0 {
				...
				m.Encodings = append(m.Encodings, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				if elementCount != 0 && len(m.Encodings) == 0 {
					m.Encodings = make([]Encoding, 0, elementCount)
				}
				for iNdEx < postIndex {
					var v Encoding
					for shift := uint(0); ; shift += 7 {
