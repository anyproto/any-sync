...
func TestPeerService_DialWebTransport(t *testing.T) {
	...
	t.Run("webtransport present without webrtc", func(t *testing.T) {
		...
		for i, s := range schemes {
			...
			if s == transport.WebTransport {
				wtIdx = i
			}
		}
		// webrtc is not registered in the fixture, but webtransport should be present
		assert.Equal(t, -1, webrtcIdx)
		require.NotEqual(t, -1, wtIdx)
	})
