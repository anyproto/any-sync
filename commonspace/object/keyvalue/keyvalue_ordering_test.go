...
// keyOf extracts the logical key from a "<key>-<peerId>" id. Keys used in these
// tests contain no dash, so splitting on the first dash yields the key.
func keyOf(id string) string { return strings.SplitN(id, "-", 2)[0] }
