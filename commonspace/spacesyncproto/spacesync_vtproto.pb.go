...
func (m *StoreKeyInner) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.DeletePrefix) > 0 {
		i -= len(m.DeletePrefix)
		copy(dAtA[i:], m.DeletePrefix)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.DeletePrefix)))
		i--
		dAtA[i] = 0x3a
	}
	if len(m.Key) > 0 {
...
func (m *StoreKeyInner) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 7:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field DeletePrefix", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.DeletePrefix = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
