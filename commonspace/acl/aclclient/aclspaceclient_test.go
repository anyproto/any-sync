...
func TestAclSpaceClient_RevokeAndRotate(t *testing.T) {
	expectSend := func(fx *fixture) {
	...
	t.Run("RevokeInviteAndRotate revokes the one invite and rotates the read key", func(t *testing.T) {
		fx := newFixture(t)
		defer fx.finish(t)
		require.NoError(t, fx.exec.Execute("a.invite_anyone::inviteId,r"))
		invites := fx.acl.AclState().Invites()
		require.Len(t, invites, 1)
		inviteId := invites[0].Id
		oldKey, err := fx.acl.AclState().CurrentReadKey()
		require.NoError(t, err)

		newKey := crypto.NewAES()
		newPrivKey, _, err := crypto.GenerateRandomEd25519KeyPair()
		require.NoError(t, err)
		expectSend(fx)
		require.NoError(t, fx.RevokeInviteAndRotate(ctx, inviteId,
			list.ReadKeyChangePayload{MetadataKey: newPrivKey, ReadKey: newKey}))

		require.Empty(t, fx.acl.AclState().Invites(aclrecordproto.AclInviteType_AnyoneCanJoin))
		cur, err := fx.acl.AclState().CurrentReadKey()
		require.NoError(t, err)
		require.True(t, cur.Equals(newKey))
		require.False(t, cur.Equals(oldKey))
	})

	t.Run("RevokeAllInvitesAndRotate revokes every invite and rotates the read key", func(t *testing.T) {
		fx := newFixture(t)
		defer fx.finish(t)
		...
		require.NoError(t, err)
		expectSend(fx)
		require.NoError(t, fx.RevokeAllInvitesAndRotate(ctx,
			list.ReadKeyChangePayload{MetadataKey: newPrivKey, ReadKey: newKey}))
